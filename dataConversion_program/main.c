/*
    ...
    ファイル構成は、プログラムと同じ階層の'beforeConversionFiles'フォルダの中に読込み用のcsvファイルを置いておくこと。


    - dataConversion_program.c (このプログラム)
    /beforeConversionFiles
        - pointData_A_1.csv
        - pointData_A_2.csv
        - pointData_A_3.csv
        - ...（読み込み用csv）
...
int main(void){
    ...
    for(int fileNum = 0; fileNum < sizeof(readFileName) / sizeof(readFileName[0]); fileNum++){
        ...
        //ファイルを読み込む
        char path[60] = "beforeConversionFiles/";   //現在のパス
        strcat(path, readFileName[fileNum]);
        if ((fp_read = fopen(path, "r")) == NULL) {
